...
from app.account.services import change_password, create_user, authenticate_user, email_verification_send,verify_email_token, verify_password_reset_token, password_reset_email_send
...
async def login(session: SessionDep, user_login: UserLogin):
    ...
    response = JSONResponse(content={"message": "Login Succsesful"})
...
async def verify_email(session: SessionDep, toekn: str):
    return await verify_email_token(session, toekn)

@router.post("/change-password")
async def change_password(session: SessionDep, data: PasswordChangeRequest, user: User= Depends(get_current_user)):
...
async def logout(session: SessionDep, request: Request, user: User = Depends(get_current_user)):
    ...
    response = JSONResponse(content={"detail": "Logout Succeessfuly"})
